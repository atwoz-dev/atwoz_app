...
class _MultiBtnSelectDialogState extends ConsumerState<MultiBtnSelectDialog> {
  ...
  void initState() {
    super.initState();
    final idealTypeState = ref.read(idealTypeNotifierProvider).idealType;
    _selectedItems = widget.title == '지역'
        ? List<String>.from(idealTypeState.regions)
        : List<String>.from(idealTypeState.hobbies);
  }
  ...
  void _submit() {
    ...
    notifier.selectMultiType(widget.title, _selectedItems);
    pop(context);
  }
