...
class RootProfileMenuWidget extends ConsumerWidget {
  const RootProfileMenuWidget({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final appStateAsync = ref.watch(globalProvider);
      ...
      data: (appState) {
        final menuItems = [
          GestureDetector(
            ...
            child: _buildItem(IconPath.circle, "테마 변경", context),
          ),
          GestureDetector(
            onTap: ref.read(authServiceProvider).signOut,
            child: _buildItem(IconPath.circle, "로그아웃", context),
          ),
        ];
  ...
  Future<void> _changeThemeMode(
    ...
    final appNotifier = ref.read(globalProvider.notifier);
        ...
        children: [
          DecoratedBox(
            ...
            child: DefaultIcon(
              icon,
              colorFilter:
                  DefaultIcon.fillColor(context.theme.colorScheme.primary),
            ),
          ),
          const Gap(14),
          Expanded(
